...
export const SocialInfo = ({ nickName, followCount, img, isFollow }: SocialInfoProps) => {
	...
	// isFollow prop이 변경될 때 state 동기화
	useEffect(() => {
		setIsFollowing(isFollow)
	}, [isFollow])

	const handleFollow = async () => {
		follow({ targetId: Number(id), follow: !isFollowing })
	...
	return (
		<div className='px-6 pt-6 pb-4'>
			<div className='flex bg-gradient-to-r from-purple-50 to-blue-50 p-5 items-center gap-6 shadow-lg rounded-2xl border border-purple-100 hover:shadow-xl transition-shadow duration-300'>
				<img
					className='w-16 h-16 rounded-full border-3 border-white shadow-md hover:scale-110 transition-transform duration-200'
					src={img ? `${baseUrl}/img/${img}` : UserImage}
					alt='프로필 이미지'
				/>
