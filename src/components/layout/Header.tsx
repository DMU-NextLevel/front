...
export const HeaderMain: React.FC = () => {
	...
	// Apple 스타일 스크롤 처리 - 스크롤 시 메뉴 즉시 닫기 및 헤더 숨김/표시
	useEffect(() => {
		const handleScroll = () => {
			...
			if (currentScrollY > lastScrollY && currentScrollY > 200) {
				// 아래로 스크롤 중이고, 스크롤 위치가 200px 이상일 때 헤더 숨김
	...
	}, [isMenuOpen, isMobileMenuOpen, location.pathname, lastScrollY])	// 알림 외부 클릭 처리
	useEffect(() => {
	...
	useEffect(() => {
		const handleClickOutside = (e: MouseEvent) => {
			if (searchFormRef.current && !searchFormRef.current.contains(e.target as Node) && isSearchExpanded) {
				setIsSearchExpanded(false)
			}
		}
				...
				<div className='px-4 sm:px-6 md:px-[8%] lg:px-[10%] xl:px-[12%] 2xl:px-[15%]'>
					<div className='flex items-center h-14'>
						...
						<nav className='hidden lg:flex items-center ml-8 space-x-8'>
							...
							>
								<div className={`flex items-center space-x-1 ${location.pathname === '/' && !isScrolled ? 'text-white hover:text-gray-200' : 'text-gray-900 hover:text-gray-600'} transition-colors cursor-pointer py-2`}>
									<i className='bi bi-list w-4 h-4 leading-none'></i>
									<span className='text-sm font-medium leading-none'>메뉴</span>
								</div>
							</div>

							{/* 다른 메뉴 아이템들 */}
							<a href={createSearchLink('RECOMMEND')} className={`text-sm font-medium px-3 py-2 rounded-lg transition-all duration-300 ${location.pathname === '/' && !isScrolled ? 'text-white hover:text-gray-200 hover:bg-white/10' : 'text-gray-900 hover:text-gray-600 hover:bg-gray-100'} `}>
								인기
							</a>
							...
							<form 
								...
								<div className={`overflow-hidden transition-all duration-500 ease-out ${isSearchExpanded ? 'w-full opacity-100' : 'w-0 opacity-0'}`}>
									<input
										...
										className={`bg-transparent border-none h-[35px] w-full text-[15px] pl-2 transition-colors duration-300 focus:outline-none ${location.pathname === '/' && !isScrolled ? 'text-white placeholder:text-white/50' : 'text-gray-800 placeholder:text-gray-400'}`}
									/>
								</div>
							</form>
						...
						{!isLoggedIn ? (
							<>
								<button
									...
									className={`p-2 rounded-full transition-all duration-300 hover:scale-110 ${location.pathname === '/' && !isScrolled ? 'text-white hover:bg-white/20' : 'text-gray-900 hover:bg-gray-100'}`}
									title="로그인"
								>
								...
								{/* <div className='relative'>
									<div className='opacity-50 cursor-not-allowed'>
										<i
											...
											} ${location.pathname === '/' && !isScrolled ? 'text-white' : 'text-gray-600'}`}></i>
									</div>
									{showNotification && (
								...
								<div className='relative inline-block' onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
									...
									<div
										...
										<div className='bg-gradient-to-r from-[#5e60ce] to-[#4361ee] rounded-xl p-5 text-center text-white relative overflow-hidden'>
											...
											/>
											<div className='font-bold text-[17px] relative z-10'>{user?.nickName || '익명'}</div>
											{/* 프로필 뱃지 */}
										...
										<div className='mt-4 space-y-3'>
											<div className='flex items-center text-sm text-gray-700 px-2'>
												<i className='bi bi-envelope text-gray-400 mr-2 text-base'></i>
										...
										<div className='flex justify-around'>
											<div onClick={() => navigate('/mypage')} className='flex flex-col items-center text-xs text-gray-600 cursor-pointer hover:text-blue-600 hover:bg-blue-50 rounded-lg p-3 transition-all duration-200 group'>
												<i className='bi bi-person text-lg mb-1 group-hover:scale-110 transition-transform duration-200'></i>
												마이페이지
											</div>
											<div onClick={handleLogout} className='flex flex-col items-center text-xs text-gray-600 cursor-pointer hover:text-red-600 hover:bg-red-50 rounded-lg p-3 transition-all duration-200 group'>
												<i className='bi bi-box-arrow-right text-lg mb-1 group-hover:scale-110 transition-transform duration-200'></i>
												로그아웃
											</div>
											<div onClick={() => navigate('/mypage')} className='flex flex-col items-center text-xs text-gray-600 cursor-pointer hover:text-purple-600 hover:bg-purple-50 rounded-lg p-3 transition-all duration-200 group'>
												<i className='bi bi-gear text-lg mb-1 group-hover:scale-110 transition-transform duration-200'></i>
												내정보 변경
											</div>
										</div>
									</div>
				...
				<div className='px-4 sm:px-6 md:px-[8%] lg:px-[10%] xl:px-[12%] 2xl:px-[15%] py-8'>
					<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8'>
							...
							<div className='space-y-1'>
								{categories.slice(0, 10).map((cat) => (
									<button
										...
										className={`flex items-center space-x-2 w-full text-left py-0.5 ${location.pathname === '/' && !isScrolled ? 'text-white hover:text-blue-400' : 'text-gray-700 hover:text-blue-600'} transition-colors`}
									>
						...
						<div>
							...
							<div className='space-y-1'>
								<a href={createSearchLink('RECOMMEND')} className={`block text-xs ${location.pathname === '/' && !isScrolled ? 'text-white hover:text-blue-400' : 'text-gray-700 hover:text-purple-600'} transition-colors py-0.5`}>
									인기 프로젝트 보기
								</a>
								<a href={createSearchLink('RECOMMEND')} className={`block text-xs ${location.pathname === '/' && !isScrolled ? 'text-white hover:text-blue-400' : 'text-gray-700 hover:text-purple-600'} transition-colors py-0.5`}>
									추천 프로젝트
